...
def main():
    ...
    # 提取你要保存的输出路径作为一个变量，方便统一管理
    output_images_dir = '/data/yjy_data/DDBM_GT_Unet/result_S2O_text_290000'
    ...
    for i, data in enumerate(dataloader):
        ...
        batch_all_exist = True
        for fname in x0_filename:
            file_name = os.path.basename(fname)
            file_path = os.path.join(output_images_dir, file_name)
            if not os.path.exists(file_path):
                batch_all_exist = False
                break  # 只要有一个不存在，就需要重新推理这个 batch

        if batch_all_exist:
            logger.log(f"Batch {i} 已经推理过，跳过...")
            continue
        ...
        #     tensor2image(x, i)
        save_images_with_filenames(path[200],x0_filename,output_images_dir)
...
def create_argparser():
    defaults = dict(
        ...
        rho=7.0,
        steps=100,
        model_path="/data/yjy_data/DDBM_GT_Unet/logs_S2O_Canny_CAIB_MSFM_scene_text_encoder/ema_2_0.9999_290000.pt",
        CNW_path="/data/yjy_data/DDBM_GT_Unet/logs_S2O_Canny_CAIB_MSFM_scene_text_encoder/ema_1_0.9999_290000.pt",
